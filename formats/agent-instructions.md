...
tools: ['<tool1>', '<tool2>']
model: '<preferred model or prioritized list>'
handoffs:
...
- **Naming**: Filenames should be lowercase, hyphen-separated, ending
  in `.agent.md` (e.g., `security-reviewer.agent.md`). Any `.md` file
  in `.github/agents/` is treated as an agent definition.
