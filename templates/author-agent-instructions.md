...
description: >
  ...
  or .github/skills/*/SKILL.md CLI skills. Also supports Codex
  (AGENTS.md), Claude Code (CLAUDE.md), and Cursor (.cursorrules).
...
params:
  platform: "Target agent platform(s): 'GitHub Copilot', 'Claude Code', 'Codex', 'Cursor', or 'All'"
...
output_contract:
  ...
  description: >
    ...
    skills, produces .github/skills/*/SKILL.md. For Claude Code, Codex,
...
For Claude Code, Codex, and Cursor, combine all content into a single
file (these platforms do not support per-file skill targeting).
...
3. **Codex** (`AGENTS.md`):
   - No strict size limit documented; prefer completeness over brevity
   - Use clear Markdown structure; Codex reads the full file as
...
5. **All platforms** (when `{{platform}}` is `All`):
...
3. **Write combined files** for Claude Code / Codex / Cursor (if targeted):
